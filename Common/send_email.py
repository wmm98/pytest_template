...
def basic_email():
    username = 'mingmingwu1998@126.com'
    password = 'wu980109..'

    auth = 'CGIXAVDEHBHZOOXZ'
    ...
    # 登录，需要：登录邮箱和授权码
    smtp.login(user=username, password=auth)

    # 构造MIMEText对象，参数为：正文，MIME的subtype，编码方式
    message = MIMEText('atukoon 邮件发送测试...', 'plain', 'utf-8')
    message['From'] = Header("mingmingwu", 'utf-8')  # 发件人的昵称
    message['To'] = Header("mingmingwuqq", 'utf-8')  # 收件人的昵称
    message['Subject'] = Header('Python SMTP 邮件测试', 'utf-8')  # 定义主题内容
    print(message)

    try:
        smtp.sendmail(from_addr=username, to_addrs="792545884@qq.com", msg=message.as_string())
        print("邮件发送成功")
    except:
        print("邮件发送失败")
